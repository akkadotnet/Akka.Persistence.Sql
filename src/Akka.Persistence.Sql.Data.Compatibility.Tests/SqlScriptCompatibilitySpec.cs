...
namespace Akka.Persistence.Sql.Data.Compatibility.Tests
{
    ...
    {
        protected SqlScriptCompatibilitySpec(ITestOutputHelper output) : base(output) { }
        ...
        protected override async Task InitializeTestAsync()
        {
            ...
            var migrationSetup =
                await File.ReadAllTextAsync(Path.Combine(workingDir!, ScriptFolder, "1_Migration_Setup.sql"));

            var migration =
                await File.ReadAllTextAsync(Path.Combine(workingDir!, ScriptFolder, "2_Migration.sql"));

            var migrationCleanup =
                await File.ReadAllTextAsync(Path.Combine(workingDir!, ScriptFolder, "3_Post_Migration_Cleanup.sql"));
