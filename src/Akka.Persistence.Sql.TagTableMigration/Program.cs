    ...
    public static class Program
    {
        ...
        public static async Task<int> Main(params string[] args)
        {
            var root = new RootCommand(
                "A helper application to migrate legacy Akka.Persistence.Sql database tables to Akka.Persistence.Sql");
            ...
            root.SetHandler(
                async (connectionString, tableMapping, provider, schema, offset, endOffset, batchSize) =>
                {
                    var opt = new Options
                    {
